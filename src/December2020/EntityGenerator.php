...
class EntityGenerator
{
    ...
    {
        // Dismiss schema if can't be resolved to an php entity
        if (
    ...
    private function getAttribute(Trunk $schema, bool $entity = false): ?Attribute
    {
        ...
        foreach (['id', 'anchor', 'title', 'description', '$comment'] as $keyword) {
            if (isset($schema[$keyword])) {
                $attributeArgs[] = new Arg(
        ...
        // We only keep $defs of non-object or object that can't be entities (associative arrays)
        $defs = [];
        foreach ($schema['$defs']->mapValue() as $defName => $defSchema) {
        ...
        if (!$entity && $name !== ObjectSchema::class) {
            ...
            foreach (['if', 'then', 'else'] as $keyword) {
                if (isset($schema[$keyword])) {
                    $attributeArgs[] = new Arg(
                        name: new Identifier($keyword),
                        value: $this->generator->generateSchema($schema[$keyword])
                    );
                }
            }
        }
